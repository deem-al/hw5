...
def bisection_root(function, LB, UB):
    ...
    if function(LB) * function(UB) > 0:
        raise ValueError("no root between points on same side of x-axis")
...
class DTree:
    ...
    def no_repeats(self):
        '''Analyzes the tree and returns True if and 
        only if there are no “repeats”, False otherwise.'''
